...
(function(process) {
  ...
  var node = ["child_process", "cluster", "crypto", "dns", "domain", "event_emitter", "fs", "http", "https", "net", "os", "path", "punycode", "querystring", "readline", "repl", "smalloc", "stream", "string_decoder", "tls", "tty", "url", "util", "vm", "zlib"];
  ...
  NodeSystem.prototype.read = function read(location, charset) {
    ...
    return Q.ninvoke(FS, "readFile", path, charset || "utf8").catch(function(error) {
  ...
  NodeSystem.findSystem = function findSystem(directory) {
    ...
    return Q.ninvoke(FS, "stat", descriptionLocation).then(function(stat) {
      return stat.isFile();
    }, function(error) {
      return false;
  ...
  NodeSystem.findSystemLocationAndModuleId = function findSystemLocationAndModuleId(path) {
    ...
    return self.findSystem(directory).then(function(packageDirectory) {
