...
(function(process) {
  ...
  function bundleSystemId(system, id) {
    return system.load(id).then(function() {
      ...
      var payload = "[" + bundle.map(function(module) {
        ...
        return "[" + JSON.stringify(module.id) + "," + JSON.stringify(module.dirname) + "," + JSON.stringify(Identifier.basename(module.filename)) + "," + JSON.stringify(dependencies) + "," + "function (require, exports, module, __filename, __dirname){\n" + text + "\n}" + "]";
      }).join(",") + "]";
      var main = system.lookup(id);
      return (boilerplate + "(" + payload + ")" + "(" + JSON.stringify(main.filename) + ")");
    });
  }
  exports.bundleLocationId = bundleLocationId;
  function bundleLocationId(location, id) {
    return System.load(location, {node: true}).then(function(buildSystem) {
  ...
  function bundleFile(path) {
    return System.findSystemLocationAndModuleId(path).then(function(pair) {
      return bundleLocationId(pair.location, pair.id);
    });
  }
  exports.main = main;
  function main() {
    return bundleFile(Path.resolve(process.cwd(), process.argv[2])).then(function(bundle) {
      console.log(bundle);
    });
  }
